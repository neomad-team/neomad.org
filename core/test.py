...
class UtilsTest(TestCase):
    def test_clean_html_default(self):
        ...
        self.assertEqual(clean_html(
            ...
            '<p>&lt;script&gt;</p>')
        self.assertEqual(clean_html(
            '<h1 class="bigger">title</h1>'), '<p>title</p>')
        self.assertEqual(clean_html(
            '<h2 class="bigger">title</h2>'), '<h2>title</h2>')
        self.assertEqual(clean_html('<p>&nbsp;&nbsp;</p>'), '')

    def test_clean_html_custom(self):
        allowed_tags = {'a': ('href'), 'h3': (), 'img': ('src')}
        self.assertEqual(clean_html(
            '<p>content</p>', allowed_tags), '<p>content</p>')
        self.assertEqual(clean_html(
            '<h2 class="bigger">title</h2>', allowed_tags), '<p>title</p>')
        self.assertEqual(clean_html(
            '<h3 class="bigger">title</h3>', allowed_tags), '<h3>title</h3>')
        self.assertEqual(clean_html(
            '<img src="/image.jpg">', allowed_tags), '<img src="/image.jpg"/>')
